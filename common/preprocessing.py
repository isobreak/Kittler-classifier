...
def get_tumor_contour(img: numpy.ndarray,  solve_hair: bool = True,
    ...
    if len(best_contours) == 0:
        ...
        best_ratio = cv2.arcLength(best_contours[0], True) / cv2.contourArea(best_contours[0])
        best_mode = 0
        for mode, contour in enumerate(best_contours):
            ratio = cv2.arcLength(contour, True) / cv2.contourArea(contour)
            if ratio <= best_ratio:
                best_ratio = ratio
                best_mode = mode
...
def get_cropped_image(img: numpy.ndarray, size_step: int = 250, solve_hair: bool = True,
                      tumor_only: bool = False, draw_contour: bool = False,
                      contour_color: tuple[int, int, int] = (255, 0, 0),
                      area_ratio_thresh: float = 0.65, indent_ratio_thresh: float = 0.18) -> numpy.ndarray:
    '''
    :param img: source image
    :param size_step: size increase step
    :param solve_hair: if hair-problem should be solved (decreases quality of segmentation, but ignores hair)
    :param tumor_only: return only the tumor (without background)
    ...
    contour = get_tumor_contour(img, solve_hair=solve_hair, area_ratio_thresh=area_ratio_thresh, indent_ratio_thresh=indent_ratio_thresh)
    ...
    while w > size or h > size:
        ...
        if size > HEIGHT or size > WIDTH:
            size -= size_step
            w -= size_step
            print(fr'Area (h={h}, w={w}) could not fit in image with such STEP_SIZE ({size_step})')
            break
